...
export default function App() {
  ...
  // Seed a demo recipe when the app loads with nothing (runs once)
  const seedInjectedRef = useRef(false);
  useEffect(() => {
    if (!isActiveRecipeCacheReady || !isSavedRecipeCacheReady) return;
    if (seedInjectedRef.current) return;
            ...
            return (
              ...
              >
                <div className="relative group">
                  ...
                  >
                    <TimerAnimationIcon type={animType} size={24} className={t.remaining === 0 ? 'text-red-500' : 'text-forest-dark cm-light-surface-ink'} />
                    <span className={`text-[10px] font-mono font-bold ${t.remaining === 0 ? 'text-red-500' : 'text-forest-dark cm-light-surface-ink'}`}>
                    ...
                    >
                      <div className="flex items-center justify-center mb-4">
                        <div className={`p-4 rounded-full ${t.remaining === 0 ? 'bg-red-500/20 text-red-500' : 'bg-forest-dark/10 dark:bg-black/10 text-forest-dark cm-light-surface-ink'}`}>
                          <TimerAnimationIcon type={animType} size={48} />
                        </div>
                      </div>
