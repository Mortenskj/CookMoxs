  ...
  return (
    ...
    >
      <div className="flex justify-between items-start mb-2">
        <h3 className="text-xl font-serif text-forest-dark dark:text-white group-hover:text-forest-mid dark:group-hover:text-white/80 transition-colors pr-4 italic">
          {recipe.title}
        </h3>
        {recipe.isFavorite ? <Heart size={18} className="text-forest-mid dark:text-white/70 fill-current shrink-0" /> : null}
      </div>

      {recipe.summary ? (
        <p className="text-sm text-forest-mid dark:text-white/70 line-clamp-2 mb-4 leading-relaxed opacity-80">{recipe.summary}</p>
      ) : null}

      <div className="flex items-center justify-between text-xs font-bold uppercase tracking-widest text-forest-mid dark:text-white/70">
        ...
        <ChevronRight size={16} className="text-forest-mid dark:text-white/70 group-hover:translate-x-1 transition-transform" />
      </div>

      <div className="mt-3 flex items-center justify-between gap-3">
        {savedAt ? (
          <div className="text-[11px] text-forest-mid/70 dark:text-white/50">
